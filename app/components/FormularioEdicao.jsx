...
function FormularioEdicao({ itemEditado, onCancel, onSave }) {
  ...
  const handleInputChange = (fieldName, value) => {
    setFormData({ ...formData, [fieldName]: formatarDocumento(value) });
  };
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`https://call-list-api.onrender.com/person/${itemEditado._id}`, {
        ...
        body: JSON.stringify({
          ...formData,
          documento_identificacao: limparMascara(formData.documento_identificacao) // Removendo a máscara
        }),
      });
  ...
  return (
    <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'rgba(255, 255, 255, 0.9)', padding: '20px', borderRadius: '50px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)', zIndex: 9999 }}>
      ...
      <TextField label="Celular" value={formData.celular} onChange={(e) => setFormData({ ...formData, celular: e.target.value })} fullWidth inputProps={{ maxLength: 11 }} style={{ marginBottom: '5px' }} />
      ...
      <TextField label="Telefone Comercial" value={formData.comercial} onChange={(e) => setFormData({ ...formData, comercial: e.target.value })} fullWidth inputProps={{ maxLength: 10 }} style={{ marginBottom: '5px' }} />
      <TextField label="Outros" value={formData.outros} onChange={(e) => setFormData({ ...formData, outros: e.target.value })} fullWidth inputProps={{ maxLength: 11 }} style={{ marginBottom: '5px' }} />
